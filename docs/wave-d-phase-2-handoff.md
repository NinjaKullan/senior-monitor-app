...
GATE: paste this to CC only after the template kettle_ask_parent_v3
shows **Approved** in Twilio (Content Template Builder → template →
WhatsApp approval status). v1 and v2 are dead — Meta forbids emoji
in buttons (DECISIONS 205); v3 is the buttonless plain-text ask,
submitted 2026-08-30 and In review on Meta. If it shows Rejected or
Paused instead, STOP — bring the exact reason text back to the PM.
...
- Approved template: kettle_ask_parent_v3, Content SID
  HXee3060b2784a551bffda9d12dbb07b86, language en, category Utility,
  zero variables, type twilio/text — NO buttons (DECISIONS 205:
  Meta forbids emoji in buttons; parents reply with 👍 themselves,
  as the body invites). Build nothing that expects a button payload;
  the reply arrives as an ordinary inbound message.

...
| D3 | DONE 2026-09-13 (founder ruling): anchors to a public-sector allowlist are admitted (`site/src/lib/citationOrigins.ts`: NIA, CDC, CFPB, FEMA, USFA, ACL, Eldercare Locator, Medicare); both standalone tests strip exactly those and still refuse a vendor host, a rel, a target or a foreign stylesheet. No page names a source yet, so no link exists yet; the writer adds citations at the next content pass | was: absolute-URL ban |
...
| D5 | DONE 2026-09-13: `/blog/` has a description (live). Resources already had one; privacy and terms left bare on purpose | was: copy law |
| D6 | DONE 2026-09-13: one Organization record on the home page (name, legal name, url, logo, email, founder first name, sameAs linkabitai.com), live and valid JSON. LinkedIn URL to be added to sameAs once A6 exists. No Article markup | was: script ban |
...
| D8 | DONE 2026-09-13: Cloudflare's robots.txt management is off (Security → Settings → Manage your robots.txt → Disable), and the served file matched the repo within a minute. The repo file now states the policy: everyone allowed, `Content-Signal: search=yes, ai-input=yes, ai-train=no`, Disallow for GPTBot, ClaudeBot, CCBot, Bytespider, Google-Extended, Applebot-Extended, meta-externalagent, Amazonbot. `robots.test.ts` refuses a search or answer agent in the blocked set | was: Cloudflare override |
...
| 2026-09-13 | D3, D5, D6, D8 deployed and verified live: robots.txt equals the repo file, one valid Organization record on the home page, /blog/ description served, www still 301s | curl against heykettle.com | | D4 remains (title em dash), PM copy call |

...
| T3 | Cloudflare injects two scripts into every page: `email-decode.min.js` (Email Address Obfuscation) and the `challenge-platform/scripts/jsd` bot-detection beacon | The page no longer "fetches nothing"; the `mailto:` is rewritten to `/cdn-cgi/l/email-protection` so non-JS readers see "[email protected]"; the beacon is a Cloudflare telemetry call, which sits badly beside LAW-9 even though it is not analytics | Email Address Obfuscation turned off 2026-09-12 in the founder's Cloudflare session and verified live (plain mailto restored, no email-decode script). The challenge-platform beacon remains: on the Free plan JavaScript Detections cannot be disabled on its own, and Bot Fight Mode was already off; Cloudflare community threads document the same. Recorded as a platform limit. Backlog A3 |
...
| T14 | Cloudflare's "Block AI bots" managed rule is on with the preference "Mixed purpose crawlers will be blocked on September 15" selected. Cloudflare's own announcement names Googlebot, Applebot and Bingbot as mixed-purpose crawlers that will be blocked where training is blocked, by default on pages that display ads | If the ads heuristic misfires on a site with no ads, the whole site drops out of Google, Bing and Apple three days after this audit. The safe setting for a site whose objective is to be found is "continue to be allowed" | Founder decision, urgent (before 2026-09-15). Backlog A8 |
| T15 | Cloudflare serves a managed robots.txt in place of the repo's: it adds `Content-Signal: search=yes,ai-train=no,use=reference` and disallows Amazonbot, Applebot-Extended, Bytespider, CCBot, ClaudeBot, CloudflareBrowserRenderingCrawler, Google-Extended, GPTBot and meta-externalagent. The repo file says "Everything here is meant to be found" | Search crawlers are still allowed. AI assistants that cite sources (a discovery channel Search Console now reports on) are refused; whether that is wanted is a policy question, not an SEO one | Decision D8 |

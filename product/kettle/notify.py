"""Ops notification transport (ntfy.sh).

Product law #3: there is no code path here that can reach a family member or a
monitored parent. One topic, the founder's, from an env var. The family-facing
ladder is spec 004 and is not built.
"""
...
class Notifier(Protocol):
    """Anything that can deliver an ops alert."""
...
class NtfyNotifier:
    ...
    The topic string is the only authentication ntfy has, so it is treated as a
    secret: env-supplied, never logged, never rendered anywhere.
    """
    ...
    def send(self, message: str) -> bool:
        """POST the message to the ops topic. Never raises."""
        if not self._topic:

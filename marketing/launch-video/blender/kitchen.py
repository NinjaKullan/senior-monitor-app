"""Mom's kitchen: shots 2 (a busy morning), 6 (a morning that isn't hers) and 7 (Kettle asks; a thumbs up).
...
def build(shot: str) -> None:
    ...
    P.card("window", "assets/layers/window.png", 0.25, -0.02, 0.595, z=0.56)   # low, clear of the caption band
    ...
    # Her phone on a little paper stand, warm screen to the camera. Shot 6: dark, the day not started.
    ...
    if shot == "02":                               # slow push-in; steam rises; her phone lights up
        ...
        P.key(steam, "location", n, (steam.location.x + 0.01, steam.location.y, steam.location.z + 0.03))
        ...
        slip = P.box("slip", P.PAPER, (0.07, 0.003, 0.045), (PHONE[0], PHONE[1] - 0.03, TOP + 0.35), (-14, 0, 0))

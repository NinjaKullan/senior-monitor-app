...
*PM: Fable, 2026-08-15. Written from the Amma field log (DECISIONS 103–107) and the setup
...
- **The child wizard** (in the child app): provisions parents, chooses signals, and emits one
  ...
  link to the right parent. The child never handles files (DECISIONS 102).
...
1. **Merged method is the default** (DECISIONS 107, 109): per parent, two shortcuts and two
...
2. **Step zero is the Shortcuts app** (DECISIONS 103). The page's first screen checks-by-asking
   ("Do you see this icon?") with the App Store link right there. Never assume the app exists.
3. **Nobody reads** (DECISIONS 106). One visual per step, one short paragraph maximum, speech
   bubbles over prose. Every screen answers "what do I tap next", nothing else.
4. **Remote eyes are a first-class assist** (DECISIONS 104). The page works when the person
...
5. **Run Immediately, always** (DECISIONS 107). Both automation screens show the toggle
   explicitly; Run After Confirmation is a dead automation on a parent's phone.
6. **The scary prompt is pre-empted** (DECISIONS 99). The screen before first run shows
   Apple's warning verbatim with "your phone will ask this; here is why it is safe".
7. **Consent is carried by the product, not the call** (DECISIONS 106). The page opens with
...
8. **Verification is prediction** (runbook §8, DECISIONS 92). The final step: "open WhatsApp
...
## 3. The fork — RESOLVED (DECISIONS 117, 2026-08-16)
...
- iCloud link generation stays rejected (programmatic sharing unsupported; DECISIONS 101/102
  standing rule).
...
5. Signals chosen in the wizard from the parent's actual habits — the load-bearing question
   ...
   browser-signal consent sentence when applicable (DECISIONS 94 note). Financial apps
...
2. Every parent-side tap enumerated honestly in the test script; ≤ 40 total from opening the
   ...
   *(Amended per DECISIONS 121, 2026-08-16: the original ≤ 12 assumed automations could ship
...
5. Field-noted: first stranger family's install produces a numbered DECISIONS block, same as
   the founder's own.

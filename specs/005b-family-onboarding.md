...
## 3. The fork — §5.1, close before building delivery

**If** iOS opens an HTTPS-served signed `.shortcut` straight into the Add Shortcut sheet
(experiment queued to Claude Code, one tap to answer):

- The setup page's two buttons ARE the shortcuts. Tap → Add Shortcut. Delivery is done.

**If not:**

- Fallback chain, in order of preference: (a) page instructs the one extra hop honestly
  (download → tap in Files), with the hop counted in the tap budget; (b) iCloud shortcut
  links generated by the signing service — never via the founder's Shortcuts library
  (standing rule, QUESTIONS 101).

# Decisions — archive, items 1 through 120

Append-only history. Nothing in this file is edited or reworded; items arrive here
verbatim once they are far enough behind that the live file no longer needs to carry
them, and they keep the numbers they were filed under.

**Load this file only when a cited number falls between 1 and 120.** Everything from
121 onward, and the next-number convention, live in `specs/DECISIONS.md`.

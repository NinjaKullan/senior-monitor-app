...
2. Health logic: device_alive stale beyond 26h reads `Not heard in a while`; an app signal at 3 days reads `Connected` (within 7-day window) with `3 days ago` recency; a signal with no pings reads `Not set up yet` with no recency beside it (amended by the ruling on QUESTIONS 60 and the on-device round at 68 — a never-reported parent shows zero amber, no nudge and no recency words; one real ping then 8 stale days shows amber and the nudge).
